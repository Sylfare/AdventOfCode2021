...
class Grid{
    ...
    isCompleted(index){
        if(this.completedAt != -1) return null;
...
for (let index = 0; index < numbers.length; index++) {
    ...
    grids.forEach((grid)=>{
        ...
        if(completed){
            grid.completedAt = index;
            remaining--
            if(remaining == 0){
                // find the latest grid
                //let final = grids.reduce((prev,cur)=>prev.completedAt < cur.completedAt ? prev=cur : null, {completedAt:-1000});

                console.log("final score : ", grid.calculateScore(number));
                finished = true;
            }
            return;
        }
